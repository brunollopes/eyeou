      ...
      if ($contest.users.indexOf(user_id) > -1) {
        if ($contest.type == 'free') {
          Image.find({
            ...
            return $images.length >= 1 ? res.status(403).send('Authorization Error: You can\'t upload more than 1 image to this contest') : next()
          })
        } else {
          try {
            ...
            if ($transaction) {
              return $images.length >= $transaction.maxPhotosLimit ? res.status(403).send('Authorization Error: You can\'t upload more images') : next()
            } else {
